...
Module_GSMData_T module_gsmdata;

s32 GetModuleSIMData(void)
{
	s32 ret;
	ret =RIL_GetIMEI(module_gsmdata.IMEI);
	if(ret!=0)
	 	{
	 		APP_DEBUG("RIL_GetIMEI Error");
	 	}
	ret =RIL_SIM_GetCCID(module_gsmdata.CCID);
	if(ret!=0)
	 	{
	 		APP_DEBUG("RIL_SIM_GetCCID Error");
	 	}
	ret =RIL_SIM_GetIMSI(module_gsmdata.IMSI);
	if(ret!=0)
	 	{
	 		APP_DEBUG("RIL_SIM_GetIMSI Error");
	 	}
	APP_DEBUG("module_data.imei=%s\r\n",module_gsmdata.IMEI);
	APP_DEBUG("module_data.ccid=%s\r\n",module_gsmdata.CCID);
	APP_DEBUG("module_data.imsi=%s\r\n",module_gsmdata.IMSI);
	return ret;
}
