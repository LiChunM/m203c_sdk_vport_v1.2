...
u8 ScanBlecom(u8 op, u8* name, u32 timeout)
{
	...
	//Ql_memset(strAT, 0x0, sizeof(strAT));
    //Ql_sprintf(strAT, "AT+QGATCFGSCAN=320,80");
    //ble_scan_print("%s", strAT);
    //ret = Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), ATRsp_QBTGatsreg_Hdlr_203C, NULL, 0);
...
static bool is_uuid_present(const ble_gap_evt_adv_report_t* p_adv_report,u8 *Ble_name)
{
    ...
    for (idx = 0; idx < p_adv_report->ble_gap_adv_info_num; idx++)
    {
       
        // Local Name
        if (p_adv_report->peer_adv_info[idx].type == BLE_GAP_AD_TYPE_COMPLETE_LOCAL_NAME)
        {
            for (i = 0; i < p_adv_report->peer_adv_info[idx].dlen; i++)
            {
				ble_scan_print("%02c", p_adv_report->peer_adv_info[idx].data[i]);
				Ble_name[i]=p_adv_report->peer_adv_info[idx].data[i];
            }
	...
	if(p!=NULL)
		{
			...
			Ql_memset(Ble_name,0,sizeof(Ble_name));
			is_uuid_present(&adv_report,Ble_name);
			Ql_sprintf(rssi,"%d",adv_report.rssi);
			Add_adv_report2List(Ble_name,adv_report.peer_addr.addr,rssi);
		}
#endif
	 ...
	 p1 = Ql_strstr(strURC, "+QBTGATCSCAN: \"ABCD\",");
 	 p2 = Ql_strstr(p1, "\r\n");
	 if (p1 && p2)
        {
        	len = p2 - p1;
	 		if(len>40&&len<200)
	 			{
		            ...
		            Ql_memcpy(strTmp, p1,len);
					ble_at_print("%s\r\n",strTmp);
					ble_adv_report_encode(&adv_report,strTmp);
					Ql_memset(Ble_name,0,sizeof(Ble_name));
					is_uuid_present(&adv_report,Ble_name);
					Ql_sprintf(rssi,"%d",adv_report.rssi);
					Add_adv_report2LongList(Ble_name,adv_report.peer_addr.addr,rssi);
	 			}
        }
