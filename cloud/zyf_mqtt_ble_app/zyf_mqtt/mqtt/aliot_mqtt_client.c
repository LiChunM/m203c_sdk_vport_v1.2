...
static int MQTTKeepalive(amc_client_t* pClient)
{
    ...
    if (SUCCESS_RETURN != rc) {
        aliot_platform_mutex_unlock(pClient->lock_write_buf);
        /*ping outstanding, then close socket unsubscribe topic and handle
        * callback
        * function*/
                         ...
                         aliot_mqtt_event_handle_func_fpt messageHandler, void* pcontext)
{
    ...
    list_node_t* node = NULL;
    /*
    * NOTE: It prefer to push the element into list and then remove it when send
    * failed,
    ...
    * */
    ...
    if ((rc = amc_send_packet(c, c->buf_send, len, &timer)) != SUCCESS_RETURN) {
...
static int amc_push_pubInfo_to(amc_client_t* c, int len, unsigned short msgId, list_node_t** node)
{
    ...
    if (NULL == repubInfo) {
        aliot_platform_mutex_unlock(c->lock_list_pub);
        ALIOT_LOG_ERROR("alloc memory error!");
        return FAIL_RETURN;
    }
                               ...
                               list_node_t** node)
{
    ...
    if (NULL == subInfo) {
        aliot_platform_mutex_unlock(c->lock_list_sub);
        ALIOT_LOG_ERROR("malloc %d bytes fail",sizeof(amc_subsribe_info_t) + len);
        return FAIL_RETURN;
    }
...
// remove the list element specified by @msgId from list of wait
...
static int amc_read_packet(amc_client_t* c, aliot_time_t* timer, unsigned int* packet_type)
{
    ...
    if (0 == rc) {
        ...
        *packet_type = 0;
        ...
        ALIOT_LOG_ERROR("mqtt read error, rc=%d", rc);
        return FAIL_RETURN;
    }
    len = 1;
    ...
    if ((rem_len > 0) && ((rem_len + len) > c->buf_size_read)) {
        ...
        if (!remainDataBuf) {
            ALIOT_LOG_ERROR("malloc %d bytes fail", remainDataLen + 1);
            return FAIL_RETURN;
        }
...
static int amc_handle_recv_SUBACK(amc_client_t* c)
{
    ...
    for (i = 0; i < AMC_SUB_NUM_MAX; ++i) {
        ...
        if ((NULL != c->sub_handle[i].topic_filter)
            ...
            // if subscribe a identical topic and relate callback function, then
            // ignore this subscribe.
            return SUCCESS_RETURN;
        }
    }
...
static int amc_handle_recv_PUBLISH(amc_client_t* c)
{
    ...
    if (1 != MQTTDeserialize_publish(&topic_msg.dup,
                                     &topic_msg.qos,
                                     &topic_msg.retain,
                                     &topic_msg.packet_id,
                                     &topicName,
...
static int amc_handle_recv_UNSUBACK(amc_client_t* c)
{
    ...
    for (i = 0; i < AMC_SUB_NUM_MAX; ++i) {
        if ((c->sub_handle[i].topic_filter != NULL)
            ...
            /* NOTE: in case of more than one register(subscribe) with different
            * callback function,
            *       so we must keep continuously searching related message
            * handle. */
        }
    }
    if (NULL != c->handle_event.h_fp) {
...
static int amc_cycle(amc_client_t* c, aliot_time_t* timer)
{
    ...
    default:
        ALIOT_LOG_ERROR("INVALID TYPE %d", packetType);
        return FAIL_RETURN;
    }
    return rc;
}
...
static aliot_network_t aliot_network;
...
// remove node of list of wait subscribe ACK, which is in invalid state or
// timeout
...
static void amc_keepalive(amc_client_t* pClient)
{
    ...
    do {
        ...
        if (AMC_STATE_DISCONNECTED_RECONNECTING == currentState) {
            ...
            if (SUCCESS_RETURN != rc) {
                ALIOT_LOG_ERROR("reconnect network fail, rc = %d", rc);
            } else {
                ALIOT_LOG_WARN("network is reconnected!");
                amc_reconnect_callback(pClient);
                pClient->reconnect_param.reconnect_time_interval_ms = AMC_RECONNECT_INTERVAL_MIN_MS;
            }
            break;
        }
        /*If network suddenly interrupted, stop pinging packet, try to reconnect
        * network immediately*/
...
// remove node of list of wait publish ACK, which is in invalid state or
// timeout
...
static int amc_connect(amc_client_t* pClient)
{
    ...
    rc = amc_wait_CONNACK(pClient);
    if (SUCCESS_RETURN != rc) {
        (void)MQTTDisconnect(pClient);
        pClient->ipstack->disconnect(pClient->ipstack);
        ALIOT_LOG_ERROR("wait_CONNACK error=%d", rc);
...
static int amc_handle_reconnect(amc_client_t* pClient)
{
    ...
    if (!aliot_time_is_expired(&(pClient->reconnect_param.reconnect_next_time))) {
        /* Timer has not expired. Not time to attempt reconnect yet. Return
    ...
    if (SUCCESS_RETURN == rc) {
        ...
        /*if reconnect network failed, then increase
        currentReconnectWaitInterval,
        ex: init currentReconnectWaitInterval=1s,  reconnect failed then 2s .4s.
        8s*/
...
/************************  Public Interface ************************/
static amc_client_t amc_client;
void* aliot_mqtt_construct(aliot_mqtt_param_t* pInitParams)
{
    aliot_err_t err;
    amc_client_t* pclient = &amc_client;
...
void aliot_mqtt_yield(void* handle, int timeout_ms)
{
    ...
    do {
        ...
        if (SUCCESS_RETURN == rc) {
            // check list of wait publish ACK to remove node that is ACKED or
            // timeout
            if (SUCCESS_RETURN != MQTTPubInfoProc(pClient)) {
                ALIOT_LOG_WARN("check publish fail");
            }
            // check list of wait subscribe(or unsubscribe) ACK to remove node
            // that is
            // ACKED or timeout
            if (SUCCESS_RETURN != MQTTSubInfoProc(pClient)) {
                ALIOT_LOG_WARN("check subscribe fail");
            }
        }
        // Keep MQTT alive or reconnect if connection abort.
        amc_keepalive(pClient);
    } while (SocketBufRdBit);
}
...
int32_t aliot_send_ping(void)
{
	unsigned char ping[2];
	ping[0]=0xc0;
	ping[1]=0x00;
	return aliot_platform_tcp_write(1,ping,2,0);
}
